...
#include<stdio.h>
#include<math.h>
int Transform(int p);//数字转化成十六进制字符 
int main() {
	char Array[40];//创建数组来存转化后的字符 
	int Number;
	scanf("%d",&Number);//得到待转化的数据 Number 
	
	if(Number == 0) {//特殊情况优先讨论 
		printf("0\n");
		return 0;
	}
	
	int temp = abs(Number);//temp 获取 Number 的绝对值 
	
	int count = 0;//用来计算转化出来的十六进制串长度 
	while(temp != 0) {
		int n = temp % 16;//取模运算，获取转化后数据的最低位 
		Array[count ++] = Transform(n);//将转化为十六进制字符后的 n 存进数组中 
		temp /= 16;//更新 temp 
	}
	
	if(Number < 0) putchar('-');//如果 Number 为负数，此处输出负号 
	for(int i = count - 1;i >=0;i --) { //倒序输出数组 
		printf("%c",Array[i]);
	}
	putchar('\n');
...
int Transform(int p) {//将传进来的0~15转化成'0'~'9'或者'A'~'F' 
	if(p < 10) return p + '0' ;
	else return p = 'A' + p - 10;
} 
